...
qv.util = {
	...
	dom: {
		...
		createSpan: function (parent, id, className, styleObj, onClick, text) {
			return qv.util.dom.createElement(parent, "SPAN", id, className, styleObj, onClick, text);
		},
	...
	renderError: function (qViewer, errMsg) {
		...
		else // Aplicación generada
		{
			...
			var table = qv.util.dom.createTable(container, "", { width: this.fixSize(qViewer.Width) + "px", height: this.fixSize(qViewer.Height) + "px", borderColor: "silver", borderWidth: "thin", borderStyle: "solid" });
			var tr = qv.util.dom.createRow(table);
			var td = qv.util.dom.createCell(tr, 1, "center", {}, gx.getMessage("GXPL_QViewerError") + ": " + errMsg);
		}
	},
...
qv.pivot = (function () {
	...
	function renderPivotTable(qViewer) {
		...
		var chart = qViewer.userControlId() + "_" + qViewer.ObjectName.replace(/,/g, "").replace(/\./g,"") + "_pivot_chart";
		...
		if (qViewer.RealType == "PivotTable") {
			...
			qv.util.dom.createDiv(container, chart, "", "", {}, "");
		} else {
...
qv.chart = (function () {
	...
	function FillHeaderAndFooter(charts, arrOptions)
	{
		...
		function CreateHeaderItemContainer(parent, paddingLeft, paddingRight) {
			var styleObj = { display: "table-cell" };
			if (paddingLeft)
				styleObj.paddingLeft = "3px";
			if (paddingRight)
				styleObj.paddingRight = "3px";
			var div = qv.util.dom.createDiv(parent, "", "", "", styleObj, "");
			return div;
		}

		function CreateZoomItem(parent, text, id) {
			var headerItem = CreateHeaderItemContainer(parent, true, true);
			qv.util.dom.createAnchor(headerItem, id, { cursor: "pointer" }, text);
			return headerItem;
		}
		...
		function CreateHeader(parent, qViewer, include1m, include2m, include3m, include6m, include1y, showYears, showSemesters, showQuarters, showMonths, showWeeks, showHours, showMinutes) {
			var qViewerId = qViewer.userControlId();
			var divTable = qv.util.dom.createDiv(parent, "", "QVTimelineHeader", "", { display: "table" }, "");
			var divRow = qv.util.dom.createDiv(divTable, "", "", "", { display: "table-row" }, "");
			var headerItem;
			headerItem = CreateHeaderItemContainer(divRow, true, true);
			qv.util.dom.createText(headerItem, gx.getMessage("GXPL_QViewerJSChartZoom"));

			// Zoom
			if (include1m) {
				CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToOneMonth"), qViewerId + "_Zoom_1m");
				if (include2m) {
					CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToTwoMonth"), qViewerId + "_Zoom_2m");
					if (include3m) {
						CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToThreeMonth"), qViewerId + "_Zoom_3m");
						if (include6m) {
							CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToSixMonth"), qViewerId + "_Zoom_6m");
							if (include1y) {
								CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToOneYear"), qViewerId + "_Zoom_12m");
							}
						}
					}
				}
			}
			CreateZoomItem(divRow, gx.getMessage("GXPL_QViewerJSChartZoomLevelToAll"), qViewerId + "_Zoom_0m");

			// Filler
			qv.util.dom.createDiv(divRow, "", "", "", { display: "table-cell", width: "100%" }, "");

			// Group by
			headerItem = CreateHeaderItemContainer(divRow, false, true);
			qv.util.dom.createSpan(headerItem, optionsId(qViewerId) + "_groupby_text", "", { whiteSpace: "nowrap" }, null, gx.getMessage("GXPL_QViewerGroupBy"));
			headerItem = CreateHeaderItemContainer(divRow, false, true);
			CreateGroupByCombo(headerItem, qViewer, showYears, showSemesters, showQuarters, showMonths, showWeeks, showHours, showMinutes);

			// Compare to
			headerItem = CreateHeaderItemContainer(divRow, true, true);
			qv.util.dom.createInput(headerItem, optionsId(qViewerId) + "_compare_enable", "checkbox", { verticalAlign: "sub" });
			headerItem = CreateHeaderItemContainer(divRow, false, true);
			qv.util.dom.createSpan(headerItem, optionsId(qViewerId) + "_compare_text", "", { whiteSpace: "nowrap" }, null, gx.getMessage("GXPL_QViewerCompareWith"));
			headerItem = CreateHeaderItemContainer(divRow, false, true);
			var select = qv.util.dom.createSelect(headerItem, optionsId(qViewerId) + "_compare_options");
...
qv.card = (function () {
	...
	function getMinMaxTable(parent, text1, text2, text3) {
		...
		span = qv.util.dom.createSpan(td, "", "qv-card-max-and-min-value", {}, null, text1);
		tr = qv.util.dom.createRow(table);
		td = qv.util.dom.createCell(tr, 1, "", styleObj, "");
		span = qv.util.dom.createSpan(td, "", "qv-card-max-and-min-time", {}, null, text2);
		tr = qv.util.dom.createRow(table);
		td = qv.util.dom.createCell(tr, 1, "", styleObj, "");
		span = qv.util.dom.createSpan(td, "", "qv-card-max-and-min-title", {}, null, text3);
		return table;
	}
	...
	function renderCard(qViewer) {
		...
		var width = "";
		var height = "";
		if (!gx.lang.gxBoolean(qViewer.AutoResize)) {
			width = qViewer.Width + "px";
			height = qViewer.Height + "px";
		}
		var tableOuter = qv.util.dom.createTable(container, qv.util.GetContainerControlClass(qViewer), { width: width, height: height });		// Contenedor del UC
		...
		for (var i = 0; i < qViewer.Metadata.Data.length; i++) {
			...
			if (qViewer.IncludeTrend && !qViewer.IncludeSparkline && data.LinearRegression.AnyTrend && anyRows)
				TrendDiv(td2, qViewer, data);
			td2 = qv.util.dom.createCell(tr2, 1, "center", {}, "");
			var span2 = qv.util.dom.createSpan(td2, "", elementValueClass, styleObj, onClick, valueStr);
			if (xDataField != "") {
				tr2 = qv.util.dom.createRow(table2);
				td2 = qv.util.dom.createCell(tr2, 2, "", { textAlign: "center" }, "");
				span2 = qv.util.dom.createSpan(td2, "", "qv-card-element-time", {}, null, ageStr);
			}
			tr1 = qv.util.dom.createRow(table1);
			td1 = qv.util.dom.createCell(tr1, 1, "center", {}, "");
			span1 = qv.util.dom.createSpan(td1, "", "qv-card-element-title", {}, null, datum.Title);
			if (qViewer.IncludeSparkline && xDataField != "" && anyRows) {
				...
				if (qViewer.IncludeTrend && data.LinearRegression.AnyTrend && anyRows)
					TrendDiv(td2, qViewer, data);
			}
			if (qViewer.IncludeMaxAndMin && xDataField != "" && anyRows) {
				...
				var table3;
				table3 = getMinMaxTable(td2, valueOrPercentage(qViewer, data.MinValue, datum, decimals), age(data.MinWhen), gx.getMessage("GXPL_QViewerAggregatorsMinimum"));
				td2 = qv.util.dom.createCell(tr2, 1, "", {}, "");
				table3 = getMinMaxTable(td2, valueOrPercentage(qViewer, data.MaxValue, datum, decimals), age(data.MaxWhen), gx.getMessage("GXPL_QViewerAggregatorsMaximum"));
			}
		}
		if (qViewer.IncludeSparkline && xDataField != "" && anyRows) {
			for (var i = 0; i < qViewer.Metadata.Data.length; i++) {
